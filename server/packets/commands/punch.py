...
class PunchCommand(Collection):
    ...
    @Command(name="effect2")
    async def effect2_handler(self, ctx: ServerContext, effect_id: int) -> None:
        setattr(ctx.player, "effect", effect_id)
        print(effect_id)
        ctx.player.send_log(f"`oSet effect to `5{effect_id}`o!")
